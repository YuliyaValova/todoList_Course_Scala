...
object PersistentModel extends Model:
  ...
  def delete(id: Id): Boolean =
    val tasks = loadTasks().toMap
    val option:Option[Any] = Option((saveTasks(Tasks(tasks.filter { (itemId, itemTask) => itemId != id }))))
    option match {
      case _: Some[_] => true
  ...
  def tasks(tag: Tag): Tasks =
    val tasks = loadTasks().toMap
    Tasks(tasks.filter((itemId,itemTask)=>itemTask.tags.contains(tag)))

  def complete(id: Id): Option[Task] =
    val tasks = loadTasks().toMap
    val updatedTasks = tasks.map { (itemId, itemTask) =>
      if itemId == id then
        itemId -> itemTask.complete
      else
        itemId -> itemTask
    }
    saveTasks(Tasks(updatedTasks))
  ...
  def tags: Tags =
    val tasks = loadTasks().toMap
    Tags(tasks.toList.flatMap((itemId,itemTask)=>itemTask.tags).distinct)
